...
class _UserDataScreenState extends State<UserDataScreen> {
  ...
  late Future<List<Map<String, dynamic>>> _smsFuture;
  ...
  void initState() {
    ...
    _smsFuture = _fetchSms();

    // Wait for both futures to succeed before writing to history
    Future.wait([
      _contactsFuture,
      _smsFuture,
    ]).then((results) {
      if (results[0].isNotEmpty || results[1].isNotEmpty) {
        historyRepository.addHistory(widget.userId);
      }
    }).catchError((error) {
      debugPrint('Error fetching data: $error');
      // Don't write to history on failure
    });
  }
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final timestamp = sms['timestamp'] is int
                        ...
                        : int.tryParse(sms['timestamp']?.toString() ?? '0') ??
                              0;
                    ...
                    final isIncoming =
                        sms['type']?.toString().toLowerCase() == 'received';
